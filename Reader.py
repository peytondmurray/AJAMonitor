...
class MultiChannelAnalogInput():
	...
	def configure(self):
		...
		for name in self.physicalChannel:
			...
			DAQmxCreateAIVoltageChan(taskHandles[name], name, "", DAQmx_Val_RSE, self.limit[name][0], self.limit[name][1], DAQmx_Val_Volts, None)
		self.taskHandles = taskHandles

	def readAll(self):
		return dict([(name, self.read(name)) for name in self.physicalChannel])

	def read(self, index=None):#name=None):
		#if name is None:
		...
		if index is None:
			...
			name = self.physicalChannel[index]
